...
Date: 2025-03-20
Description: Main script to run the ECASP toolkit.
"""

import argparse
import sys
from ecasp import header
from ecasp.create_data import create_datafile, create_dataset, verify_h5_file
from ecasp.train import train
# from ecasp.test import test
from ecasp.calibrate import calibrate
from ecasp.transfer import transfer
from ecasp.predict import predict
from ecasp.variant import variant
from ecasp.scripts import gradient_rbp_attribution
from ecasp.scripts import prepare_rbp_expression
from ecasp.train_base import utils as train_utils

__VERSION__ = header.__version__

def parse_args_create_data(subparsers):
    parser_create_data = subparsers.add_parser('create-data', help='Create datasets for ECASP model training')
    ...
    parser_create_data.add_argument('--val_split_ratio', type=float, default=0.1, help='Ratio of training and validation dataset')
...
def parse_args_train(subparsers):
    parser_train = subparsers.add_parser('train', help='Train an ECASP model')
    ...
    parser_train.add_argument(
        ...
        type=int,
        default=10,
        help="T_0 for CosineAnnealingWarmRestarts in epochs (default: 10; ignored unless --scheduler CosineAnnealingWarmRestarts).",
    )
    ...
    parser_train.add_argument("--head-dropout", type=float, default=0.1, help="Dropout probability applied before final output head (Dropout1d)")
...
def parse_args_test(subparsers):
    parser_test = subparsers.add_parser('test', help='Test an ECASP model')
    ...
    parser_test.add_argument('--test-target', '-t', default="ECASP", choices=["ECASP", "SpliceAI-Keras"], type=str)
...
def parse_args_calibrate(subparsers):
    parser_calibrate = subparsers.add_parser('calibrate', help='Calibrate an ECASP model')
...
def parse_args_transfer(subparsers):
    parser_transfer = subparsers.add_parser('transfer', help='Fine-tune a pretrained model with ECASP.')
    ...
    parser_transfer.add_argument("--head-dropout", type=float, default=0.1, help="Dropout probability applied before final output head (Dropout1d)")
    parser_transfer.add_argument(
        "--film-dropout",
        type=float,
        default=None,
        help="Dropout probability inside FiLM MLP (default: 0.2). Set 0 to disable.",
    )
    parser_transfer.add_argument(
        "--film-noise-std",
        type=float,
        default=None,
        help="Noise std added to FiLM input during training (default: 0.05). Set 0 to disable.",
    )
...
def parse_args_predict(subparsers):
    parser_predict = subparsers.add_parser('predict', help='Predict splice sites in a given sequence using an ECASP model')
    ...
    parser_predict.add_argument('--film-strength', type=float, default=1.0, help='Scale FiLM gamma/beta at inference (>1 strengthens, <1 weakens; 1 keeps original).')
...
def parse_args_variant(subparsers):
    ...
    parser_variant.add_argument('--film-strength', type=float, default=1.0, help='Scale FiLM gamma/beta at inference (>1 strengthens, <1 weakens; 1 keeps original).')
...
============================================================
ECASP
Expression-Conditioned AI for Splicing Prediction
============================================================
    '''
