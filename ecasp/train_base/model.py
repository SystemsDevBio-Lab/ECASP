...
from ecasp.rbp.metadata import decode_rbp_metadata, encode_rbp_metadata
...
class SpliceAI(nn.Module):
    def __init__(self, L, W, AR, apply_softmax=True, film_config=None, head_dropout: float = 0.1):
        ...
        if head_dropout < 0.0 or head_dropout >= 1.0:
            raise ValueError(f"head_dropout must be in [0.0, 1.0); got {head_dropout}.")
        self.head_dropout = nn.Dropout1d(p=head_dropout) if head_dropout > 0.0 else nn.Identity()
    ...
    def forward(self, x, rbp_embedding=None):
        ...
        final_x = self.head_dropout(final_x)
