...
from ecasp.constants import *
from ecasp.rbp.expression import load_rbp_expression
from ecasp.train_base.model import *
from ecasp.train_base.utils import *
...
def initialize_model_and_optim_transfer(device, flanking_size, epochs, scheduler,
    ...
    if rbp_expression_path and not disable_film:
        ...
        if film_dropout is not None:
            film_config["film_dropout"] = float(film_dropout)
        if film_noise_std is not None:
            film_config["film_noise_std"] = float(film_noise_std)
    ...
    model = SpliceAI(L, W, AR, film_config=film_config, head_dropout=head_dropout).to(device)
    ...
    if scheduler == "MultiStepLR":
        ...
        if cosine_t0 <= 0:
            raise ValueError(f"cosine_t0 must be a positive integer; got {cosine_t0}.")
        scheduler_obj = torch.optim.lr_scheduler.CosineAnnealingWarmRestarts(
            optimizer, T_0=cosine_t0, T_mult=1, eta_min=1e-6, last_epoch=-1)
...
def transfer(args):
    print("Running ECASP with 'transfer' mode")
    ...
    model, optimizer, scheduler, params, rbp_context = initialize_model_and_optim_transfer(
        ...
        disable_film=args.nofilm, lr=args.lr, film_lr_mult=args.film_lr_mult, head_dropout=args.head_dropout,
        film_dropout=args.film_dropout, film_noise_std=args.film_noise_std, cosine_t0=args.cosine_t0)
...
def run_split_multi_mixed(model, tissues, device, params, loss_name, metric_files,
    ...
    steps_per_epoch = max(1, (virtual_total_batches + accumulate_steps - 1) // accumulate_steps)
    base_epoch = global_batch_idx // steps_per_epoch
    ...
    for round_idx in range(max_batches):
        ...
        for tissue in order:
            ...
            if accum_counter >= accumulate_steps:
                optimizer.step()
                if scheduler is not None and not is_plateau_scheduler:
                    epoch_fraction = base_epoch + (processed_batches / max(1, virtual_total_batches))
                    scheduler.step(epoch_fraction)
                    current_lr = scheduler.get_last_lr()[0]
                    with open(metric_files['learning_rate_every_batch'], 'a') as f:
                        f.write(f"{current_lr}\n")
