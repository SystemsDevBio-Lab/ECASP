...
from ecasp.predict.predict import *
from ecasp.predict.utils import *
from ecasp.rbp.metadata import extract_film_config_from_state_dict
from ecasp.train_base.model import SpliceAI
...
def load_pytorch_models(model_path, CL, film_strength: float = 1.0):
    """
    ...
    for state_dict_obj, source_path in models:
        try: 
            ...
            if hasattr(model, "film_strength"):
                model.film_strength = float(film_strength)
...
class Annotator:
    ...
    def __init__(self, ref_fasta, annotations, model_path='SpliceAI', model_type='keras', CL=80, rbp_context=None, rbp_tensor=None, film_strength: float = 1.0):
        """
        ...
        if model_path == 'SpliceAI':
            ...
            self.models = load_pytorch_models(model_path, CL, film_strength=film_strength)
            self.keras = False
        else:
