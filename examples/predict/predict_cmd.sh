#!/bin/bash
# A script to run the ECASP predict command via `ecasp predict`
...
# Build the ECASP predict command
CMD="ecasp predict -m "$MODEL_PATH" -o "$OUTPUT_PATH" -f $FLANKING_SIZE -i "$DATA_PATH""
